...
export default function CategoryManagementPage() {
  ...
  const handleAdd = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (categories.some((category) => category.slug === normalizedSlug)) {
    ...
    setCategories((prev) => [
      ...prev,
      { id: normalizedSlug, name: normalizedName, slug: normalizedSlug },
    ]);
    setName('');
    setSlug('');
  };
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-charcoal-light">
                {categories.map((category) => (
                  <tr key={category.id} className="hover:bg-gray-50 dark:hover:bg-charcoal/50 transition-colors">
                    ...
                    <td className="p-4 text-gray-500 dark:text-gray-400 text-sm">{category.slug}</td>
