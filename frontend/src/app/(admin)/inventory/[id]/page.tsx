...
import { ArrowLeft, Plus, Trash2, Star, Save } from 'lucide-react';
...
export default function EditProductPage() {
    ...
    const fetchProduct = async () => {
      ...
      try {
        ...
        if (response.data) {
          ...
          setSlug((product.slug as string) || '');
          setDescription((product.description as string) || '');
          setPrice(String(product.price ?? ''));
          setCurrency((product.currency as string) || 'USD');
          ...
          // Load images
          const imagesResponse = await apiClient.getProductImages(id);
          if (imagesResponse.data) {
            const productImages = (imagesResponse.data as Array<Record<string, unknown>>).map(
              (img: Record<string, unknown>) => ({
                id: img.id as string,
                originalUrl: img.originalUrl as string,
                isThumbnail: img.isThumbnail as boolean,
  ...
  const handleSetThumbnail = (imgId: string) => {
    setImages((prev) =>
      prev.map((img) => ({
        ...img,
        isThumbnail: img.id === imgId,
      }))
    );
  };
    ...
    try {
      ...
      for (const img of images) {
        if (img.file) {
          ...
          await apiClient.uploadImage(id, formData);
        }
      }
