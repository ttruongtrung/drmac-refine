...
const categoryDefaultFields: Record<string, Array<{ id: string; label: string }>> = {
  macbook: [
...
export default function CreateProductPage() {
  ...
  const initializeDefaultFields = (catId: string) => {
    const defaults = categoryDefaultFields[catId] ?? [];
    setDynamicFields(defaults.map((field) => ({ ...field, value: '' })));
  };

  const handleCategoryChange = (catId: string) => {
    setCategoryId(catId);
    initializeDefaultFields(catId);
  };
